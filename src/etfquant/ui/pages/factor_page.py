...
**关于计算速度：**
当前"生成因子"是对15个预置表达式逐一计算IC/RankIC，使用pandas+scipy纯CPU运算，因此速度较快。真正的"因子挖掘"（RL搜索/遗传算法自动发现新因子）需要大量GPU计算，该功能尚未实现。
"""
...
| 参数 | 建议范围 | 说明 |
|------|----------|------|
| 搜索步数(N-steps) | 512~8192 | RL搜索总步数。512=快速验证，2048=标准搜索，4096=深度搜索，8192=穷举搜索 |
| 批大小(batch_size) | 64~512 | 每步RL采样的因子表达式数量。越大搜索越稳定但越耗GPU内存 |
| 最大搜索时长(分钟) | 5~120 | 搜索超时保护。到达时限后自动停止并保存已发现的有效因子 |
| 最大因子数 | 10~100 | 搜索到多少个有效因子后自动停止 |

**搜索策略说明：**
- **RL搜索**：使用强化学习智能探索因子表达式空间，每步尝试生成新表达式并评估IC，逐步学习哪些算子组合更容易产生有效因子
- **遗传搜索**：模拟自然选择，对已有有效因子进行交叉变异产生新因子，适合在已有因子池基础上扩展
- 两种策略可以组合使用：先用RL搜索发现初始因子，再用遗传搜索在已有因子基础上优化

**资源消耗预估：**
| N-steps | batch_size | 预计耗时 | GPU显存占用 | 适用场景 |
|---------|-----------|---------|------------|----------|
| 512 | 64 | ~5分钟 | ~2GB | 快速验证 |
| 2048 | 128 | ~20分钟 | ~4GB | 标准搜索 |
| 4096 | 256 | ~60分钟 | ~6GB | 深度搜索 |
| 8192 | 512 | ~2小时+ | ~8GB | 论文级研究 |
"""
    ...
    with ui.tab_panels(tabs, value="pool").classes("full-width"):
        ...
        with ui.tab_panel("mining"):
            with ui.card().classes("full-width q-mb-md"):
                ...
                with ui.row().classes("full-width q-mt-md"):
                    strategy_select = ui.select(label="搜索策略", options=["RL搜索", "遗传搜索", "RL+遗传组合"], value="RL搜索").classes("q-mr-md").style("min-width: 200px")
